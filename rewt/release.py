    ...
    **Why it is generated and not written.** `.zenodo.json` already declares the
    ...
    for c in zj["creators"]:
        ...
        if c.get("affiliation"):
            # QUOTED, because this is the one field a person writes freely. An affiliation
            # containing a colon and a space — "Director: Digital", a title with a
            # subtitle — makes an unquoted scalar into a nested mapping, and the CFF
            # stops parsing. Stephen's five affiliations survived unquoted only because
            # they happen to use semicolons and parentheses; documentation checked that
            # in memory before asking for the file to be regenerated, which is how the
            # hazard was found rather than met. `yaml.safe_dump` quotes only when it must
            # and escapes correctly when it does, so the common case still reads plainly.
            # `json.dumps`, not `yaml.safe_dump`: a JSON string IS a valid YAML
            # double-quoted scalar, on one line, with the escaping already right. My
            # first attempt used safe_dump, which line-wraps a long value and produced
            # a CFF that would not parse — caught because the check below runs on the
            # generated file rather than on the intention.
            authors.append(f"    affiliation: {json.dumps(c['affiliation'])}")
    return "\n".join([
