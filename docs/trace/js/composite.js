...
/* The endpoint is a default rather than a constant, so a caller can supply its own and a
   copy of this file carries none. rewt-fc's suggestion. The tracer supplies nothing and
   uses this, under D-072: the Library's request concerns public presentation and the
   tracer presents to nobody without a token. */
const NLS_DEFAULT = 'https://mapseries-tilesets.s3.amazonaws.com/os/six-inch-';
...
async function countyTile(slug, z, x, y, base = NLS_DEFAULT) {
  try {
    const r = await fetch(`${base}${slug}/${z}/${x}/${y}.png`);
    if (!r.ok) return null;
...
export async function compositeTile(z, x, y, opts = {}) {
  ...
  const sheets = await Promise.all(here.map((r) => countyTile(r.slug, z, x, y, opts.base)));
...
/**
 * An in-memory cache in front of `compositeTile`, keyed on z/x/y for the session.
 *
 * WHY THE TRACER NEEDS ONE AND THE VIEWER DID NOT. rewt-fc's server cached composited
 * tiles to disk across sessions and users; a browser cannot. For panning a backdrop that
 * costs a few round trips and nobody notices. **The tracer reads these pixels while a line
 * is being dragged** — the livewire runs on mouse-move — so a composite costing three
 * fetches per tile cannot sit in that path. This is the cache rewt-fc told me to build on
 * my side of the boundary, because its access pattern is panning and mine is not.
 *
 * Bounded, because an ImageBitmap holds real memory and a long session over a county would
 * otherwise accumulate every tile it ever touched. Oldest out first; the working set while
 * tracing one channel is small.
 */
const CACHE = new Map();
const CACHE_MAX = 240;

export async function compositeTileCached(z, x, y, opts = {}) {
  const key = `${z}/${x}/${y}`;
...
export async function compositeTilePNG(z, x, y, opts = {}) {
  const bmp = await compositeTileCached(z, x, y, opts);
  const cv = new OffscreenCanvas(256, 256);
  if (bmp) cv.getContext('2d').drawImage(bmp, 0, 0);
  const blob = await cv.convertToBlob({ type: 'image/png' });
  /* An ArrayBuffer, not a Uint8Array view over one. MapLibre's protocol contract is for
     the buffer itself, and a typed-array view is accepted by some paths and silently
     ignored by others — which shows up as a source that reports itself loaded, a handler
     that is called for every tile, and a map that draws nothing at all. */
